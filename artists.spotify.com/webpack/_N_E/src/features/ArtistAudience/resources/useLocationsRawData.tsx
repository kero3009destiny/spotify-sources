      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            url = "".concat(S4X_DATA_API, "/v1/artist/").concat(props.artistId, "/audience/locations?time-filter=28day&aggregation-level=recording").concat(countryCode);
            _context.prev = 2;
            _context.next = 5;
...
var LocationsLoader = createLoader(load, {
  cacheKeyFn: function cacheKeyFn(props) {
    return "".concat(props.artistId, ",").concat(props.countryCode, ",");
  }
});
export var useLocationsRawData = function useLocationsRawData(country) {
  var artist = useCurrentArtist();
  var artistId = artist.id;
  var countryCode = country.code;
  useEffect(function () {
    return function cleanup() {
      LocationsLoader.clear({
        artistId: artistId,
        countryCode: countryCode
      });
    };
  }, [artistId, countryCode]);
  return useRead(LocationsLoader, {
    artistId: artistId,
    countryCode: countryCode
  });
};