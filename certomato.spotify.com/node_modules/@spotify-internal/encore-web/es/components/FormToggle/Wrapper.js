...
export var Wrapper = styled.span.withConfig({
  ...
  var checkedStyles = css(["background-color:", ";", "{right:2px;left:auto;}"], cssColorValue(props.semanticColor), Indicator); // unchecked means toggle is in the "off" position

  var uncheckedStyles = css(["background-color:", ";", "{left:2px;right:auto;}"], cssColorValue(semanticColors.essentialSubdued), Indicator);
  var focusStyles = css(["&::after{border-color:", ";}"], cssColorValue(semanticColors.essentialBase));
  var activeStyles = css(["opacity:", ";", "{width:", ";}"], opacityActive, Indicator, spacer20);
  var disabledStyles = css(["opacity:", ";cursor:", ";"], opacityDisabled, cursorDisabled);
  return css(["border-radius:", "px;height:", ";position:relative;min-width:", ";", ";input:focus ~ &{", "}", " input:active ~ &{", "}", " input:disabled ~ &{", "}input:not(:checked) ~ &{", "}input:checked ~ &{", "}"], borderRadius, spacer16, wrapperWidth, focusGapBorder("".concat(borderRadius + focusBorderWidth, "px"), props.isUsingKeyboard), focusStyles, props.isFocus && css(["input ~ &{", "}"], focusStyles), activeStyles, props.isActive && css(["input ~ &{", "}"], activeStyles), disabledStyles, uncheckedStyles, checkedStyles);
});