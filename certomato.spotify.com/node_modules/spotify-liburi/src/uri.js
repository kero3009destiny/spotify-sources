...
var _splitIntoComponents = function(str) {
  ...
  if (str.indexOf(URI_PREFIX) === 0) {
    ...
    } else if (str.indexOf(PATH_PREFIX) === 0) {
      components = str.slice(PATH_PREFIX.length).split('/');
    } else {
      throw ERROR_INVALID;
    }
    format = Format.URL;
...
var _parseFromComponents = function(components, format, query) {
  ...
  switch (part) {
    ...
    case URI.Type.LIVE:
      return URI.liveURI(_getIdComponent());
    case URI.Type.CONCERT:
      return URI.concertURI(_getIdComponent());
    case '':
      break;
    case URI.Type.SOCIAL_SESSION:
      return URI.socialSessionURI(_getNextComponent());
    case URI.Type.INTERRUPTION:
      return URI.interruptionURI(_getNextComponent());
    default:
...
/**
 * Creates a new 'live' type URI.
 *
 * @param {string} id The id of the live.
 * @return {URI} The live URI.
 */
URI.liveURI = function(id) {
  return new URI(URI.Type.LIVE, {id: id});
};
 ...
 *
 * @param {string} id The token needed to join a social session.
 * @return {URI} The socialsession URI.
 */
URI.socialSessionURI = function(id) {
  return new URI(URI.Type.SOCIAL_SESSION, {id: id});
};

/**
 * Creates a new 'interruption' type URI.
 *
 * @param {string} id The id of the interruption.
 * @return {URI} The ad URI.
 */
URI.interruptionURI = function(id) {
  return new URI(URI.Type.INTERRUPTION, {id: id});
};
...
URI.isLive = function(uri) { return (URI.from(uri) || {}).type === URI.Type.LIVE; };
...
URI.isSocialSession = function(uri) { return (URI.from(uri) || {}).type === URI.Type.SOCIAL_SESSION; };
URI.isInterruption = function(uri) { return (URI.from(uri) || {}).type === URI.Type.INTERRUPTION; };
